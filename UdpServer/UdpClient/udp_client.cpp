...
void udp_client(SOCKET s, char* ipv4, unsigned short port) {
	...
	*/
	...
	while (1) {
		...
		while (1) { //this loop makes sure that the received datagram is from the server
			// Check the server address
			SOCKADDR_IN toBeCheckAddress;
			int toBeCheckAddressSize = sizeof(toBeCheckAddress);
			result = recvfrom(s, sbuf, sizeof(sbuf), 0, (SOCKADDR*)& toBeCheckAddress, &toBeCheckAddressSize);
			// If message is sent fromt the same IP address and the same port
			if (remoteAddress.sin_port == toBeCheckAddress.sin_port
				&& remoteAddress.sin_addr.s_addr == toBeCheckAddress.sin_addr.s_addr)
				break;
		}
